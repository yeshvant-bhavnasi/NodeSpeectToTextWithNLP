setTimeout(
  function () {
    var dog = 'haha';
    var recognition;
     //let socket = io.connect("http://10.190.33.141:3000/");
    //let socket = io.connect("http://98.234.219.228:3000/");
    let socket = io.connect("http://10.190.97.219:3000/");
      ...
      bindTranscript: function (val) {
        ...
        msg = `<li class="transcription-wrapper">
          ...
          if(val.annotations.length > 0){
            ...
            suggestion.prepend(suggestionValue)
            }
          }
        ...
        $("#suggestion").on('click','.add',function(){
          ...
          if(meta == "policyNumber"){
            ...
            if(value.toLowerCase() == "yes"){
              ...
              $("label:contains('Was there equipment failure?')").next().find(`label[title=No]`).trigger( "click" );
            }
